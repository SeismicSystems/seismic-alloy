//! Receipt envelope types for Seismic.
...
pub enum SeismicReceiptEnvelope<T = Log> {
...
impl SeismicReceiptEnvelope<Log> {
    /// Creates a new [`SeismicReceiptEnvelope`] from the given parts.
    pub fn from_parts<'a>(
        status: bool,
...
impl<T> SeismicReceiptEnvelope<T> {
    /// Return the [`SeismicTxType`] of the inner receipt.
    ...
    /// Create a new [`SeismicReceiptEnvelope`] from the values that alloy's AnyReceiptEnvelope
    /// contains
...
impl<T> TxReceipt for SeismicReceiptEnvelope<T>
where
...
impl Encodable for SeismicReceiptEnvelope {
...
impl Decodable for SeismicReceiptEnvelope {
...
impl Typed2718 for SeismicReceiptEnvelope {
    fn ty(&self) -> u8 {
        let ty = match self {
            Self::Legacy(_) => SeismicTxType::Legacy,
            Self::Eip2930(_) => SeismicTxType::Eip2930,
...
impl Encodable2718 for SeismicReceiptEnvelope {
...
impl<'a, T> arbitrary::Arbitrary<'a> for SeismicReceiptEnvelope<T>
where
...
mod tests {
    ...
    fn encode_legacy_receipt() {
        ...
        let mut data = vec![];
        let receipt = SeismicReceiptEnvelope::Legacy(ReceiptWithBloom {
            receipt: Receipt {
                status: false.into(),
    ...
    fn legacy_receipt_from_parts() {
        let receipt = SeismicReceiptEnvelope::from_parts(
            true,
            100,
            vec![],
            SeismicTxType::Legacy,
        ...
        assert_eq!(receipt.tx_type(), SeismicTxType::Legacy);
    }

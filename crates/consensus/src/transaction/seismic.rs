...
impl TxSeismic {
    ...
    ///
    /// Note these are two different things. The raw `authorization_list` (the
    /// `SignedAuthorization[]` array) is not an EIP-712 field. It only lives on the RLP wire.
    /// What the schema includes is this single `bytes32` hash of that array. We commit via a
    /// hash rather than re-encoding the whole array as a typed field to keep the schema
    /// compact.
    ///
    /// That commitment is what makes the signature cover the auth list. Without it, the signing
    /// hash wouldn't depend on the auth list at all, so an attacker could append or replace
    /// authorization tuples on a signed transaction (the signature would still recover to the
    /// original signer) and grief the fee-payer with extra EIP-7702 gas. With it, tampering
    /// with the wire auth list changes this hash, changes the signing hash, and makes signer
    /// recovery yield a different address.
    ///

...
/// Container type for all Seismic specific fields in a genesis file.
...
impl OpChainInfo {
    /// Extracts the Seismic specific fields from a genesis file. These fields are expected to be
...
/// The Seismic-specific genesis block specification.
#[derive(Default, Debug, Clone, Copy, Eq, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl OpGenesisInfo {
    /// Extract the Seismic-specific genesis info from a genesis file.
    pub fn extract_from(others: &OtherFields) -> Option<Self> {
        Self::try_from(others).ok()
    }
}
...
/// The Seismic-specific base fee specification.
#[derive(Default, Debug, Clone, Copy, Eq, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl OpBaseFeeInfo {
    /// Extracts the Seismic base fee info by looking for the `optimism` key. It is intended to be
